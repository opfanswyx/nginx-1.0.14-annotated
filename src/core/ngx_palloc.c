...
#if 0
/* nginx内存池主内存 */
/* typedef struct ngx_pool_s ngx_pool_t; 定义在ngx_core.h*/
struct ngx_pool_s {
    ...
    ngx_pool_cleanup_t   *cleanup;      /* 可自定义回调函数，清楚内存块分配的内存 */
    ngx_log_t            *log;          /* 对应日志模块 */
};
#endif
ngx_pool_t *
ngx_create_pool(size_t size, ngx_log_t *log)
{
    ...
    p->d.next = NULL;       /* 下一个ngx_pool_t内存池地址 */
    p->d.failed = 0;        /* 失败次数 */
...
ngx_palloc(ngx_pool_t *pool, size_t size)
{
    u_char      *m;
    ngx_pool_t  *p;
    /* 判断每次分配的内存大小，如果超出pool->max的限制，则通过大数据内存分配策略 */
    if (size <= pool->max) {
    ...
    /* 大块数据分配策略 ，在pool->large链表上分配 */
    return ngx_palloc_large(pool, size);
}

/* 使用内存池分配一块内存2，不考虑对齐情况 */
void *
...
/* 内存池释放需要走ngx_destroy_pool，独立大内存块的单独释放，可以走ngx_pfree方法。 大内存块释放  pool->large */
ngx_int_t
